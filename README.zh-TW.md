...
- **時間控制**——播放/暫停、調整速度倍率、時光倒流、跳至指定日期。預設從真正的 1 倍速開始(v1.8:1 秒現實 = 1 秒模擬,而非先前約 86400 倍速的預設值)。速度下拉選單(v1.8.2)改成以真實時間為基準的倍率階梯——1x/2x/5x,再加上沿用之前的兩個固定倍率(0.1 天/秒 ≈ 8640x、1 天/秒 ≈ 86400x)——不再是舊的「天/秒」數字(跟新的預設值對不上,而且要自己心算才知道快了幾倍)。
