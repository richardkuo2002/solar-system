...
- **地心視差(拓撲中心)修正**——從既有的地心位置出發,扣除觀測者自身的位置(由經緯度/海拔與恆星時推算),v1.3 起改用 **WGS-84 扁球體**地球模型(不再是球形)。升降時刻 v1.3 起也計入**大氣折射**(Bennett 公式)。RA/Dec 輸出 v1.5 起採**當日平春分點**座標系(真實 IAU 歲差修正);章動與光行差仍未建模——每一項近似,以及已經改進的部分,都詳列於 [docs/accuracy.md](docs/accuracy.md#observer-mode-v06)(英文)。
