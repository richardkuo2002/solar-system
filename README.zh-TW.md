...
畫面右側(星曆 HUD 下方)的「Event Toolkit」面板是本專案的天文現象分析工具——用下拉選單挑選事件類型,共用同一組結果/圖表版面呈現。拖曳結果圖表的游標(v1.8)會暫停主時鐘,並把整個場景——不只是視線連線——跳到被拖曳到的那個時刻,放開後仍停留在那,方便你仔細檢視那一刻;要繼續播放請按時間控制列的 Play。
