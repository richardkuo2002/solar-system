...
![由上往下俯瞰的日心視角——內太陽系、小行星帶、真實星座連線,以及觀測者與逆行分析面板](docs/screenshot-topdown.png)
...
- **即時位置資料**取自 [NASA JPL Horizons](https://ssd.jpl.nasa.gov/horizons/) API,離線或 API 無法連線時會自動、無感切換回本地端的克卜勒方程式計算——渲染迴圈不會因網路請求而卡頓。畫面上的 HUD 會即時顯示目前實際使用的資料來源、參考中心與座標系,另外(v1.4)針對純克卜勒推算的天體加上「距離 J2000 元素幾年」的精度提示,詳細模型與已知限制見 [docs/accuracy.md](docs/accuracy.md)(英文)。
- **4 種攝影機模式**——每種模式都能用 WASD,但只有自由飛行是真正的自由移動,其他三種模式的 WASD 是切換該模式自己的「起始點」:
  ...
  - **行星表面第一人稱(Surface first-person)**——站在任一行星指定的經緯度上,抬頭仰望天空;WASD 走動改變經緯度(W/S=南北,A/D=東西)。太陽、該行星自己的衛星,以及(v1.4)其他每顆行星,在這個模式下都改用**真實視角徑**渲染(不是場景平常的壓縮比例)——詳見 [docs/accuracy.md](docs/accuracy.md#surface-mode-sky-realism-v13-extended-v14)(英文)。若 Event Toolkit 有分析結果,會有一個橘色標記(v1.8.1)指出被分析天體實際在畫面上的位置——真實視角徑常常會被地板成約 1.5px 的小點,不然很容易埋沒在星空裡找不到。
  - **地心視角(Geocentric)**——攝影機固定在地球位置,並保持固定的觀看方向,而地球本身沿著真實軌道移動。火星的逆行現象正是由此產生——這是真實的軌道動力學結果,而非預先寫死的動畫。WASD 切換目前追蹤的行星,並重新對準新目標。
- **時間控制**——播放/暫停、調整速度倍率、時光倒流、跳至指定日期。預設從真正的 1 倍速開始(v1.8:1 秒現實 = 1 秒模擬,而非先前約 86400 倍速的預設值)。速度下拉選單(v1.8.2,v1.8.3 擴充)改成以真實時間為基準的倍率階梯——1x/2x/5x/100x/1000x,再加上沿用之前的兩個固定倍率(0.1 天/秒 ≈ 8640x、1 天/秒 ≈ 86400x)——不再是舊的「天/秒」數字(跟新的預設值對不上,而且要自己心算才知道快了幾倍)。
- 距離與大小皆採用**冪函數曲線壓縮**(並非真實等比例),讓整個太陽系可以同時顯示在畫面上——這是網頁版太陽系視覺化的常見作法。
- **設定(v1.12)**——左下角的齒輪按鈕可切換介面語言(英文/繁體中文,
  整個 App 包含所有天文事件分析結果都已完整雙語化),還有一個音量
  滑桿會存起來但目前不接任何東西——專案還沒有音效系統可以讓它調整。
...
畫面右側(星曆 HUD 下方)的「Event Toolkit」面板是本專案的天文現象分析工具——用下拉選單挑選事件類型,共用同一組結果/圖表版面呈現。按下 Analyze(v1.10)或拖曳結果圖表的游標(v1.8)都會暫停主時鐘,並把整個場景——不只是視線連線——跳到該事件的時刻(或被拖曳到的那個時刻),放開後仍停留在那,方便你仔細檢視那一刻;要繼續播放請按時間控制列的 Play。每個事件類型會各自把上次輸入的內容存在瀏覽器的
local storage 裡,重新整理頁面後還在(v1.9)——這與下方「網址分享狀態」是分開的
機制,不會互相影響。

- **逆行運動**(v0.4;v1.5 起支援所有行星)——在每個取樣點計算目標行星的地心黃經(`λ = atan2(Δy, Δx)`,取自日心目標-地球 AU 差向量),把角度序列展開(unwrap)以避免 0°/360° 邊界誤判,再用中心差分求其變化率,最後透過粗掃描 + 二分搜尋精修出兩個變化率為零的 stationary point——絕不會直接把粗取樣點當成答案回傳。7 顆行星皆可選(包含知名的水星逆行);已用火星 2007-08 與木星 2022 的真實逆行區間對照測試。視覺呈現:主場景中的地球-目標視線連線(建議切換到日心俯瞰或自由飛行視角觀看)、對照參考網格繪出的視在路徑,以及標示兩個 stationary point 與逆行區間的 λ(t) 時間軸圖。
...
- **相位角 / 被照亮比例**(月球、水星、金星、火星)——單一時間點的計算,不需要求解器:相位角 α(太陽-目標-觀測者夾角)與被照亮比例 `k = (1 + cos(α)) / 2`。月球使用 Meeus 月球理論的位置——v1.5 起主場景 3D 月球也改用同一來源,同一日期兩邊永遠一致(詳見 [docs/accuracy.md](docs/accuracy.md#the-moons-analysis-position-v11-meeus-lunar-theory),英文)。
- **月食 / 日食**(v1.1)——建立在上述 Meeus 月球理論之上的真實食象幾何計算:月食依月球相對地球本影軸的偏移量與該距離下本影/半影錐半徑,分類為無/半影/偏食/全食;日食則針對**特定觀測地點**(經緯度/海拔)分類為無/偏食/環食/全食,計算方式與 Observer Mode 相同,採用地心視差修正。已用兩個真實歷史事件對照測試(2022-11-08 月全食、2024-04-08 日全食)。每個事件現在都會回報完整的 P1/U1/U2/U3/U4/P4 或 C1/C2/C3/C4 接觸點時刻表(v1.9),月食本影/半影半徑也套用標準的 1.01 大氣放大係數——確切的幾何簡化(球形天體、未用 Besselian elements)見 [docs/accuracy.md](docs/accuracy.md#eclipses-v11)(英文)。
- **水星/金星凌日**(v1.4)——跟日食一樣的圓盤重疊幾何,只是遮蔽天體換成行星,以內行星下合為觸發時機。已用真實的 2019-11-11 水星凌日與 2012-06-06 金星凌日對照測試。
- **行星視覺合(Appulse)**(v1.4)——兩顆選定的行星在地球天空中靠多近(地心座標);找出角距離在指定日期範圍內的每一個區域極小值。已用真實的 2020-12-21 木星土星大合對照測試。
- **月掩行星**(v1.4)——某顆行星從特定觀測地點看被月球擋住,跟日食/凌日一樣是地點相關的事件。已用真實的 2021-11-08 月掩金星(日本可見)對照測試。貼近月緣、無法解析的精度上限說明見 [docs/accuracy.md](docs/accuracy.md#lunar-occultations-v14)(英文)。
- **月球合(Moon Conjunction)**(v1.7)——月掩行星的一般化版本:某顆行星在特定觀測者天空中離月球有多近,採地心視差修正(topocentric,不同於行星視覺合的地心座標)——因為月球本身的視差幅度,常常大於這裡要回報的分離角度。每一次最接近時刻都會標記是否近到足以構成月掩(wouldOccult)。已用同一個 2021-11-08 月掩金星真實事件,以及真實的 2022-05-27 月合金星事件對照測試。
- **最佳觀測夜晚搜尋器**(v1.11)——給定一顆行星與一個觀測地點,逐夜掃描
  指定日期範圍,依三項因素把每個夜晚評成 0-100 分並排名:目標在真正天文
  暗夜(太陽高度 -18° 以下)期間能爬多高、跟地球距離的粗略遠近加分(離
  地球越近分數越高,依軌道幾何估算,不是真實星等模型)、月球是否在天上
  且亮到會干擾觀測。這是排名用的經驗法則,不是真實的光度預測——所有
  簡化說明見
  [docs/accuracy.md](docs/accuracy.md#best-observation-night-finder-v111)
  (英文)。
- **匯出**——任何分析結果都可透過面板的匯出按鈕存成 JSON 或 CSV,內含完整可重現的中繼資料(事件類型、目標、觀測者、參考座標系/中心/來源、輸入參數、求解方法與容許誤差);圖表用的密集取樣資料則刻意不包含在匯出內容中。每張顯示中的圖表(v1.7)也可以用各自的下載按鈕另存成 PNG。
- **已知限制**——僅為幾何角度;光行時修正已於 v1.7 套用到觀測者模式、行星視覺合、相位/被照亮比例,以及逆行運動的精修時刻,但衝/合、最大距角,以及食/凌日/月掩的密集掃描仍未套用(角秒等級的效應,低於其求解器容許誤差,詳見 [docs/accuracy.md](docs/accuracy.md));視在路徑圖畫的是原始 AU 座標,不是投影到天球的 RA/Dec 視圖;超出 Standish 表約 1800–2050 年有效範圍的結果會愈趨近似;不論下拉選單選哪個資料來源,每種事件的密集取樣掃描內部一律使用克卜勒傳播計算(原因見 [docs/accuracy.md](docs/accuracy.md),英文)。
...
- **地心視差(拓撲中心)修正**——從既有的地心位置出發,扣除觀測者自身的位置(由經緯度/海拔與恆星時推算),v1.3 起改用 **WGS-84 扁球體**地球模型(不再是球形)。升降時刻 v1.3 起也計入**大氣折射**(Bennett 公式)。RA/Dec 輸出是完整的**視位置**(apparent place)——真實 IAU 歲差(v1.5)、章動與周年光行差(v1.6),以及目標星的光行時修正(v1.7),採當日真春分點座標系;每一項近似,以及已經建模的部分,都詳列於 [docs/accuracy.md](docs/accuracy.md#observer-mode-v06)(英文)。
...
- **不會存**——播放速度/方向/暫停狀態、自由飛行模式的位置/視角方向、
  Event Toolkit 或 Observer Mode 的輸入(Event Toolkit 的輸入改用 local
  storage 各自記住,見上方 Event Toolkit 一節;Observer Mode 則完全不會
  被記住)。
...
- **Free-flight 垂直移動**(v1.7)——右下角 ▲/▼ 按鈕是鍵盤 Q/E 的觸控
  等效操作,採「按住」而非「點按」(因為垂直移動是連續的),補上自
  v0.10 以來的已知缺口。
- **窄螢幕版面**(v1.11.1)——螢幕寬度低於 700px(一般手機)時,左側
  Observer Mode / Planet Info Panel 跟右側 Event Toolkit 面板改成滿版
  寬度單欄顯示(而非固定 300px/340px 硬擠在兩側),並預設一開始就收合,
  避免 3D 場景一打開就被蓋住。
- **已知限制**:觸控筆電如果滑鼠是主要指標裝置,不會顯示觸控 UI(即使
  觸控實際可用)。
